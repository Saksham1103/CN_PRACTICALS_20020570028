...
char *getReminder(char *, char *); //helper function to get reminder by dividing data with polynomial
char *substr(char *, int, int); //function to get substring of a data frame
bool isFrameCorrect(char *, char *); //function to check whether the received data is correct or not	
...
void simulateNetwork(bool isNoisy)
{
    ...
    cout << "Frame received: " << transmittedFrame << endl;
    if (isFrameCorrect(transmittedFrame, genFunc))
    {
        cout << "Frame received correctly over the network" << endl;
    }
    else
    {
        cout << "Frame received incorrectly over the network" << endl;
    }
}
